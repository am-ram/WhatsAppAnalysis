...
def time_active(messages_df):
	messages_df['Time'].value_counts().head(10).plot.barh(color='cyan') # Top 10 Times of the day at which the most number of messages were sent
	plt.xlabel('Number of messages',size=20,color='yellow')
	plt.ylabel('Time',size=20,color='yellow')
	plt.tight_layout()
...
def day(messages_df):
	messages_df['Date'].value_counts().head(10).plot.barh(color='cyan')
	plt.xlabel('Number of Messages',size=20,color='yellow')
	plt.ylabel('Date',size=20,color='yellow')
	plt.tight_layout()
...
if uploaded_file is not None:
	...
	if option == "All":
		...
		st.pyplot()
		time.sleep(0.2)
		st.subheader("**%s's WordCloud **"% name);st.write('P.S : The biggest word in the plot is the most used word')
